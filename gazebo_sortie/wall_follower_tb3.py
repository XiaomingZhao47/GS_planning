...
Subscribes to: /scan (sensor_msgs/LaserScan)  [TB3 Gazebo; was /base_scan in Stage]
...
class WallFollower(Node):
    ...
    def _min_in_range(scan: LaserScan, angle_min_deg: float, angle_max_deg: float) -> float:
        ...
        for i in range(len(scan.ranges)):
            ...
            # TB3 LDS-01 publishes angles over [0, 2pi); the original Stage
            # robot used a [-pi, pi] front-centered scan. Normalize so the
            # negative-degree windows below (right=-90, front-right=-45) match.
    ...
    def _angle_to_closest(self, scan: LaserScan) -> float:
        ...
        # Normalize TB3's [0, 2pi) angle into (-pi, pi] for consistent steering.
        return (min_angle + math.pi) % (2 * math.pi) - math.pi
